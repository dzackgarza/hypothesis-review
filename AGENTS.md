...
"Send to agent", the agent reads the recorded batch. Math normalization is owned entirely
by the `h` backend at annotation intake; this tool only *consumes* `normalized_quote`
from `h` and refuses to deliver or record any annotation carrying a
`normalization_error`. The review-side normalizers (`mathquote.py`, `pdfmath.py`,
`enrich.py`) are retired.
