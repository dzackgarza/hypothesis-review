...
Annotate a docs site, paper, web app, or PDF, then turn on **Send to agent**.
The glowing toggle adds every unacted annotation in the review group to an ephemeral queue; turning it off removes the queue flag.
...
- **`annotate` CLI** — `annotate` (or `annotate queue`) prints the active queue as JSON.
  `annotate drain --item ANNOTATION_ID REMEDIATION` accepts one or more item/remediation pairs, appends them to the ledger, commits it, and marks each annotation acted.
