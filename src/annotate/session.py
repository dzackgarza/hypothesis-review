...
h write) and closes when the harness wakes it. :func:`batch_since` collects the *real*
annotations created strictly after the open timestamp -- already-``acted`` annotations and
any legacy ``review:open``/``review:send`` marker rows (a bad design, no longer emitted) are
excluded. A pure function over the ``created``-ordered annotation list.
"""
...
def batch_since(anns: list[Annotation], since: datetime) -> list[Annotation]:
    ...
    return [
        a
        for a in anns
        if a.created > since and not a.is_marker(ACTED) and not _is_legacy_marker(a)
    ]
